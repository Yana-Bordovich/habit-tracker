...
export default function EditHabitModal({ isOpen, onClose, habit }: Props) {
  const { editHabit, archiveHabit } = useHabitStore();
  ...
  const handleArchive = () => {
    archiveHabit(habit.id); // Архивируем привычку
    onClose();              // Закрываем модалку после действия
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">Редактировать привычку</h2>
          <button
            onClick={onClose}
            ...
            {colors.map((c) => (
              <button
                ...
                className={`w-12 h-12 rounded-full ${c} ${
                  color === c ? 'ring-4 ring-offset-2 ring-gray-800' : ''
                }`}
              />
            ))}
          </div>
